...
export function CompanyClientRegistrationLinksPanel({
    ...
    function deactivate(linkId: string) {
        startTransition(async () => {
            const result = await deactivateCompanyRegistrationLinkAction(
                clientId,
                linkId,
            );
            if ("error" in result) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col gap-3 rounded-lg border bg-card p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h2 className="text-sm font-medium">
                            Link de cadastro deste cliente
                        </h2>
                        <p className="text-xs text-muted-foreground">
                            Quem abrir o link preenche o formulário público vinculado
                            a este cliente (condomínio, clínica, etc.).
                        </p>
                    </div>
                    <Button
                        type="button"
                ...
                <CreateRegistrationLinkSheet
                    ...
                    title="Link de cadastro deste cliente"
                    onSubmit={async (body) => {
                        const result =
                            await createCompanyRegistrationLinkAction(
                                clientId,
                                body,
                            );
                        if ("error" in result) {
